...
class Datapath extends Module{
	...
	val fetch_stage = Module(new fetch_stage)
	val decode_stage = Module(new decode_stage)
	val execute_stage = Module(new execute_stage)
	val memory_stage = Moudle(new memory_stage)
	val writeback_stage = Module(new writeback_stage)
	...
	csr.io.de_pipe_reg.pc := decode_stage.io.de_pipe_reg.pc
	...
	fetch_stage.icache_response_ready := io.icache_response_ready
	fetch_stage.icache_response_data := io.icache_response_data
	fetch_stage.dcache_flush_tag := memory_stage.io.dcache_flush_tag
	fetch_stage.flush_fd := flush_fd
	fetch_stage.de_pipe_reg_pc_sel := decode_stage.io.de_pipe_reg.pc_sel
	fetch_stage.de_pipe_reg_inst := decode_stage.io.de_pipe_reg.inst
	fetch_stage.de_pipe_reg_enable := decode_stage.io.de_pipe_reg.enable
	fetch_stage.em_pipe_reg_pc := execute_stage.io.em_pipe_reg.pc 
	fetch_stage.flush_em := flush_em
	fetch_stage.brCond_taken := execute_stage.io.brCond_taken
	fetch_stage.jump_addr := execute_stage.io.jump_addr
	fetch_stage.io.icache <> stage.io.icache	
	...
	jump_flush := execute_stage.io.jump_flush
	jump_occur := execute_stage.io.jump_occur
