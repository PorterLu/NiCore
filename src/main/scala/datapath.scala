...
class Datapath extends Module{
	...
	val dcache_stall = (em_pipe_reg.ld_type.orR || em_pipe_reg.st_type.orR || em_pipe_reg.inst === FENCE_I) && 
						em_pipe_reg.enable && !data_cache_tag && !(( em_pipe_reg.alu_out >= "h2000000".U) && (em_pipe_reg.alu_out <= "h200ffff".U)) 
	...
	//printf(p"flush_fd:${flush_fd}; flush_de:${flush_de}; flush_em:${flush_em}; flush_mw:${flush_mw}\n")
	//printf(p"icache_stall:${icache_stall}; dcache_stall:${dcache_stall}; mul_stall:${mul_stall}; div_stall:${div_stall}\n")
	/***** Fetch ******/
	...
	/*
	csr_atomic_flush := Mux(mw_pipe_reg.enable && (mw_pipe_reg.csr_write_op =/= CSR.N) && (mw_pipe_reg.csr_write_op =/= CSR.R), Mux(stall, "b0111".U, "b1111".U), 
							Mux(em_pipe_reg.enable && (em_pipe_reg.csr_write_op =/= CSR.N) && (em_pipe_reg.csr_write_op =/= CSR.R), Mux(stall, "b0011".U, "b0111".U),
								Mux(de_pipe_reg.enable && (de_pipe_reg.csr_write_op =/= CSR.N) && (de_pipe_reg.csr_write_op =/= CSR.R), Mux(stall, "b0001".U, "b0011".U),
									Mux(fd_pipe_reg.enable && (io.ctrl.csr_cmd =/= CSR_NOP), Mux(stall, 0.U, "b0001".U), 0.U)
								)		
							)
						)*/
	csr_atomic_flush := 0.U
	...
	val next_pc = MuxCase(
		...
		(csr_atomic && (((mw_pipe_reg.csr_write_op =/= CSR.N) && (mw_pipe_reg.csr_write_op =/= CSR.R)) ||((em_pipe_reg.csr_write_op =/= CSR.N) && (em_pipe_reg.csr_write_op =/= CSR.R)))) -> csr_next_fetch,
	...
	pc := next_pc
	/*printf(p"flush:${icache_flush_tag || dcache_flush_tag}; stall:${stall}; trap:${csr.io.trap}; csr_atomic_1:" +
  p"${csr_atomic && (((mw_pipe_reg.csr_write_op =/= CSR.N) && (mw_pipe_reg.csr_write_op =/= CSR.R)) ||((em_pipe_reg.csr_write_op =/= CSR.N) && (em_pipe_reg.csr_write_op =/= CSR.R)))}; " +
  p"jump:${((de_pipe_reg.pc_sel === PC_ALU) && de_pipe_reg.enable)|| brCond_taken}\n")
	printf(p"pc:${Hexadecimal(pc)}; next_pc:${Hexadecimal(next_pc)}\n")*/
	...
	printf(p"fd_pipe_reg:\npc:${Hexadecimal(fd_pipe_reg.pc)}; inst:${Hexadecimal(fd_pipe_reg.inst)}; enable:${fd_pipe_reg.enable}\n")
	/***** Decode ******/
	//control模块输入指令
...
//	is_kill_inst := io.ctrl.is_kill && fd_pipe_reg.enable
	...
	csr.io.illegal_inst := Mux(io.ctrl.is_illegal && (!brCond_taken && !jmp_occur) && fd_pipe_reg.enable, fd_pipe_reg.inst, 0.U) 

	csr.io.fetch_misalign := ((fd_pipe_reg.pc & 3.U) =/= 0.U)  && (!brCond_taken && !jmp_occur) && fd_pipe_reg.enable
	...
	csr.io.store_misalign := (de_pipe_reg.st_type =/= ST_XXX) && 
										...
										) && ((em_pipe_reg.ld_type.orR && data_cache_tag) || !(em_pipe_reg.ld_type.orR))				//地址是否错位可能还以来load的结果
	...
	when(flush_mw && !stall){
		...
		mw_pipe_reg.jump_addr := 0.U
		mw_pipe_reg.jump_taken := false.B
		mw_pipe_reg.csr_read_data := 0.U
		mw_pipe_reg.csr_write_op := CSR.N
